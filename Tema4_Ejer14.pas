...
clrscr();
    ...
    repeat
        Write('Ingresar CODIGO del Producto [ L | M | P | A ]: ');
        ReadLn(Cod);
        Cod := UpCase(Cod);
    until (Cod = 'L') or (Cod = 'M') or (Cod = 'P') or (Cod = 'A');


    while (Cod = 'L') or (Cod = 'M') or (Cod = 'P') or (Cod = 'A') do
        begin
            repeat
            ...
            WriteLn();
            repeat
                Write('Ingresar CODIGO del Producto [ L | M | P | A ] {F : Fin}: ');
                ReadLn(Cod);
                Cod := UpCase(Cod);
            until (Cod = 'L') or (Cod = 'M') or (Cod = 'P') or (Cod = 'A') or (Cod = 'F');
    ...
    if AcumVtaMed > 0 then
        ...
        WriteLn('   Descuento     : ', -BonifMed:8:2);
        WriteLn('   Importe FINAL                 : ', AcumVtaMed * (1 - DescMed):8:2);
        end;
    ...
    if (AcumVtaAcc > 0) and (AcumVtaAcc < MinAcc) then
        begin
        TextColor(yellow);
        WriteLn(' Accesorios   MIN: $',MinAcc:4:0, ' | 0.00% OFF');
        TextColor(white);
        BonifAcc := 0;
        WriteLn('   Importe FINAL                 : ', AcumVtaAcc:8:2);
        end
    else
        if (AcumVtaAcc >= MinAcc) then
            begin
            TextColor(yellow);
            WriteLn(' Accesorios   MIN: $',MinAcc:4:0,' | ', (DescAcc*100):5:2, '% OFF');
            TextColor(white);

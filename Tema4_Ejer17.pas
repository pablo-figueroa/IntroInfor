...
repeat
    ...
    repeat
        gotoxy(1,5);
        clreol();
        Write('Ingresar cantidad de sorteos [1 a 5]: ');
        ReadLn(CantSort);
    until (CantSort > 0) and (CantSort <= 5);
    ...
    for i:=1 to CantSort do
        ...
        for j:=1 to 6 do
            ...
            repeat
                Write('  Ingresar número ',j,' de 6: ');
                Read(NumSort);
                TextColor(red);
            until ((NumSort >= 1) and (NumSort <= 40));
            TextColor(White);
    ...
    TextColor(white);
    ReadLn();
    ReadLn(); { NO SÉ PORQUE CON UNO SOLO PASA DE LARGO..! ¿?}
until FALSE;
end.